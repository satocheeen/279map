        ...
        if (mapKind === MapKind.Real) {
            ...
            dataSourceGroups.forEach(group => {
                group.dataSources.forEach(ds => {
                    if (ds.itemContents.kind === DataSourceKindType.Track) {
                        [[1, 8], [8, 13], [13, 21]].forEach(zoomLv => {
                            const layerDefine: LayerDefine = {
                                dataSourceId: ds.dataSourceId,
                                group: group.name ?? '',
                                editable: ds.itemContents.editable ?? false,
                    ...
                    } else if (ds.itemContents.kind === DataSourceKindType.Item || ds.itemContents.kind === DataSourceKindType.RealPointContent) {
            ...
            dataSourceGroups.forEach(group => {
                group.dataSources.forEach(ds => {
                    if (ds.itemContents.kind !== DataSourceKindType.Item) {
                        return;
                    }
                    [LayerType.Point, LayerType.Topography].forEach(layerType => {
                        const layerDefine: LayerDefine = {
