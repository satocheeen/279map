...
import ImageRegister from '../../common/image/ImageRegister';
...
type Props = {
    ...
    onChange: (value: api.ContentAttr) => void;
}
export default function ContentInfoForm(props: Props) {
    const [ snsPreviewPosts, setSnsPreviewPosts ] = useState<api.SnsPreviewPost[]>([]);
    ...
    const onRadioClicked = useCallback((contentType: api.ContentAttr['type']) => {
        const newValue = Object.assign({}, props.value, {
            type: contentType,
        });
        props.onChange(newValue);
    ...
    useEffect(() => {
        ...
        callApi(mapServer, api.GetSnsPreviewAPI, {
            url: props.value.url,
        })
        .then(res => {
    ...
    // @ts-ignore
    }, [props.value.type, props.value.url, mapServer]);
